...
//Setting ke 2
const nobot = "6283150930982"

// Setting Donasi
const gopay = "0821 2828 0049"
const ovo = "0821 2828 0049"
const dana = "0831 5093 0982"
const pulsa = "0821 2828 0049"
const pulsa2 = "0831 5093 0982"
const insta = "-"
const github = "Boykuu"
...
module.exports = async(conn, msg, m, setting, store) => {
    ...
    const readmore = more.repeat(4001)
		...
		const isOwner = ownerNumber == sender ? true : [`${ownerNumber}@s.whatsapp.net`, "6283171382342@s.whatsapp.net"].includes(sender) ? true : false
		...
		const buttonsDefault = [
			{ urlButton: { displayText: `GRUP Joze-BOT`, url : `https://chat.whatsapp.com/CsrcFiIWIfV8IxlA1iVumJ` } },
			{ urlButton: { displayText: `Nomer Owner`, url : `https://wa.me/6283171382342?text=Hai+kak+aku+mau+beli+PREMIUM` } },
			{ quickReplyButton: { displayText: `💰 Donasi`, id: `${prefix}donate` } },
			{ quickReplyButton: { displayText: `Script`, id: `${prefix}sc` } },
		]
		const button5 = [
			{ callButton: { displayText: `Number Owner`, phoneNumber: `083171382342` } },
			{ urlButton: { displayText: `𝙂𝙧𝙪𝙥 Joze`, url : `https://chat.whatsapp.com/CsrcFiIWIfV8IxlA1iVumJ` } },
...
if (chats.startsWith("@6283171382342")){
		...
		switch(command) {
			...
			case prefix+'help':
			  /*conn.sendMessage(from, { audio: fs.readFileSync('audio/Assalamualaika.m4a'), mimetype: 'audio/mp4', ptt: true}, {quoted: msg})*/
				...
				/*conn.sendMessage(from, { react: { text: `👋`, key: msg.key }})*/
conn.sendMessage(from, { caption: teks, image: fs.readFileSync('media/Jojo2.jpg'), templateButtons: buttonsDefault, footer: '© Joze - Bot', mentions: [sender]} )
				break
...
var teks = `  │
 ...
 conn.sendMessage(from, { caption: teks, image: {url: `https://i.ibb.co/CPcFJ6c/IMG-20220131-WA0504.jpg`}, templateButtons: donasibut, footer: '© Joze - Bot', mentions: [sender]} )  
			    break
case prefix+'sewa':
  case prefix+'daftarprem':
  var teks = `*[ LIST HARGA Joze PREM ]*
...
*LIST DAFTAR PREMIUM*
- Rp.15.000 - PERMANENT`
			    conn.sendMessage(from, { caption: teks, location: { jpegThumbnail: fs.readFileSync('media/Jojo2.jpg') }, templateButtons: button5, footer: '© Joze - Bot', mentions: [sender] })
			    break
			case prefix+'runtime':
...
case prefix+'githubown':
 ...
 break
			...
			{ quickReplyButton: { displayText: `Ubah Ke Audio`, id: `${prefix}tiktokaudio ${q}` } },
				...
				reply(mess.wait)
				conn.sendMessage(from, { caption: `Succes Download Video Tiktok, Thanks For Using Joze-BOT!`, video: {url: data.nowm}, templateButtons: tidtod, footer: '© Joze - Bot', mentions: [sender]} )
			}).catch(() => reply(mess.error.api))
			limitAdd(sender, limit)
			    break
...
case prefix+'bc': case prefix+'broadcast':
			    if (!isOwner) return reply(mess.OnlyOwner)
		            if (args.length < 2) return reply(`Masukkan isi pesannya`)
                            ...
                            for (let i of data) {
                               conn.sendMessage(i.id, { text: `*[ Joze BROADCAST ]*\n\n${q}` })
                               await sleep(1000)
                            }
                            break
...
case prefix+'add':
  if (!isOwner)return reply("_Maaf Fitur Ini Di Nonaktifkan Oleh Owner, Karena menyebabkan nomer bot 3 kali ke banned_")
    ...
    break
            ...
            case prefix+'tf':{
                 if (args.length < 2) return reply(`Kirim perintah *${command}* @tag nominal\nContoh : ${command} @6283171382342 2000`)
...
conn.sendMessage(from, { text: caption, buttons: but, footer: "© Joze Bot", templateButtons: but }, {quoted: msg})
limitAdd(sender, limit)
break
//maker arasya
case prefix+'leaves':
